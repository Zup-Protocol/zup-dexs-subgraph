...
import { handleRamsesV2PoolSwap } from "../../../../../../src/v2-pools/mappings/pool/dexs/ramses/ramses-v2-pool-swap";
...
describe("Ramses V2 Swap", () => {
  beforeEach(() => {
    clearStore();
    dataSourceMock.setNetwork(ARBITRUM_NETWORK_NAME);
  });

  test(`When calling the handler, it should get the pool fee from
    the factory and pass it converted to uniswap fee tier
    decimals to the handleV2PoolSwap function`, () => {
    let ramsesFeeTier: i32 = 25 as i32; // 25 represents 0.25% fee in ramses
    ...
    createMockedFunction(Address.fromString(V2FactoryAddress.ramses), "pairFee", "pairFee(address):(uint256)")
      .withArgs([ethereum.Value.fromAddress(pool.id)])
      .returns([ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(ramsesFeeTier))]);

    handleRamsesV2PoolSwap(event);

    assert.fieldEquals("Pool", pool.id.toHexString(), "currentFeeTier", (ramsesFeeTier * 100).toString()); // Convert to uniswap fee tier decimals (0.25% -> 2500)
  });
});
