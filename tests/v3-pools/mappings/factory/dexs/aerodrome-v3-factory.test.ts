...
export function createEvent(params: PoolCreatedEventParams = new PoolCreatedEventParams()): PoolCreated {
  ...
  createMockedFunction(event.address, "getSwapFee", "getSwapFee(address):(uint24)")
    .withArgs([ethereum.Value.fromAddress(event.params.pool)])
...
describe("aerodrome-v3-factory", () => {
  ...
  test(`When the handler is called and there
        isn't an already Aerodrome created protocol, it
        should create one with the correct values
        and assign it to the pool`, () => {
    let event = createEvent();
    handleAerodromeV3PoolCreated(event);

    assert.fieldEquals("Protocol", ProtocolId.aerodromeV3, "name", "Aerodrome V3");
    assert.fieldEquals("Protocol", ProtocolId.aerodromeV3, "url", "https://aerodrome.finance");
    assert.fieldEquals(
      "Protocol",
    ...
    assert.fieldEquals("Pool", event.params.pool.toHexString(), "protocol", ProtocolId.aerodromeV3);
  });
  ...
  test(`When the handler is called and the network is base, the position manager address should be correct`, () => {
    ...
    assert.fieldEquals(
      ...
      ProtocolId.aerodromeV3,
      "positionManager",
      Address.fromString(V3PositionManagerAddress.aerodrome).toHexString(),
    );
  });
});
